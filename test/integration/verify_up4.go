...
func buildExpectedInterfacesEntries(client *p4rtc.Client, testdata *pfcpSessionData, expectedValues p4RtValues) []*p4_v1.TableEntry {
	...
	te := client.NewTableEntry(tablesNames[p4constants.TablePreQosPipeInterfaces], map[string]p4rtc.MatchInterface{
		"n3Addr": &p4rtc.LpmMatch{
			Value: n3Addr,
			PLen:  32,
		},
...
func buildExpectedApplicationsEntry(client *p4rtc.Client, testdata *pfcpSessionData, expectedValues p4RtValues) *p4_v1.TableEntry {
	...
	matchFields := make(map[string]p4rtc.MatchInterface)
	for i, mf := range mfs {
		matchFields[fmt.Sprintf("field%d", i+1)] = mf
	}

	te := client.NewTableEntry(TableApplications, matchFields,
...
func buildExpectedSessionsUplinkEntry(client *p4rtc.Client, testdata *pfcpSessionData) *p4_v1.TableEntry {
	...
	return client.NewTableEntry(TableSessionsUplink, map[string]p4rtc.MatchInterface{
		"n3Addr": &p4rtc.ExactMatch{
			Value: n3Addr,
		},
		"teid": &p4rtc.ExactMatch{
			Value: teid,
		},
...
func buildExpectedSessionsDownlinkEntry(client *p4rtc.Client, expected p4RtValues, ueState UEState) *p4_v1.TableEntry {
	...
	return client.NewTableEntry(TableDownlinkSessions, map[string]p4rtc.MatchInterface{
		"n3Addr": &p4rtc.ExactMatch{
			Value: ueAddr,
		},
...
func buildExpectedTerminationsUplinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expected p4RtValues) *p4_v1.TableEntry {
	...
	return client.NewTableEntry(TableUplinkTerminations, map[string]p4rtc.MatchInterface{
		"ueAddr": &p4rtc.ExactMatch{
			Value: ueAddr,
		},
		"appID": &p4rtc.ExactMatch{
			Value: appID,
		},
...
func buildExpectedTerminationsDownlinkEntry(client *p4rtc.Client, testdata *pfcpSessionData, expected p4RtValues, afterModification bool) *p4_v1.TableEntry {
	...
	return client.NewTableEntry(TableDownlinkTerminations, map[string]p4rtc.MatchInterface{
		"ueAddr": &p4rtc.ExactMatch{
			Value: ueAddr,
		},
		"appID": &p4rtc.ExactMatch{
...
func buildExpectedTunnelPeersEntry(client *p4rtc.Client, testdata *pfcpSessionData) *p4_v1.TableEntry {
	...
	return client.NewTableEntry(TableTunnelPeers, map[string]p4rtc.MatchInterface{
		"tunnelPeerID": &p4rtc.ExactMatch{
...
func verifyP4RuntimeEntries(t *testing.T, testdata *pfcpSessionData, expectedValues p4RtValues, ueState UEState) {
	...
	entries, _ := p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.interfaces")
	...
	entries, _ = p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.applications")
	...
	entries, _ = p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.sessions_uplink")
	...
	entries, _ = p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.sessions_downlink")
	...
	entries, _ = p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.terminations_uplink")
	...
	entries, _ = p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.terminations_downlink")
	...
	if ueState == UEStateAttached {
		entries, _ = p4rtClient.ReadTableEntryWildcard(ctx, "PreQosPipe.tunnel_peers")
	...
	meters, _ := p4rtClient.ReadMeterEntryWildcard(ctx, MeterSession)
	...
	nrOfConfiguredMeters = 0
	meters, _ = p4rtClient.ReadMeterEntryWildcard(ctx, MeterApp)
	for _, m := range meters {
		if m.Config != nil {
			nrOfConfiguredMeters++
		}
	...
	verifyCounter := func(counterID uint32) {
		nrOfResetCounters := 0
		counters, _ := p4rtClient.ReadCounterEntryWildcard(ctx, p4constants.GetCounterIDToNameMap()[counterID])
...
func verifyNumberOfEntries(t *testing.T, tableID uint32, expectedNoOfEntries int) {
	...
	defer providers.DisconnectP4rt()
	ctx := context.Background()
	entries, err := p4rtClient.ReadTableEntryWildcard(ctx, p4constants.GetTableIDToNameMap()[tableID])
...
func verifyNoP4RuntimeEntries(t *testing.T) {
	...
	allInstalledEntries, _ := p4rtClient.ReadTableEntryWildcard(ctx, "")
	...
	for _, table := range tables {
		entries, _ := p4rtClient.ReadTableEntryWildcard(ctx, table)
...
func verifyP4RuntimeSliceMeter(t *testing.T, expectedValues p4RtValues) {
	...
	meters, _ := p4rtClient.ReadMeterEntryWildcard(ctx, sliceTcMeter)
	...
	if expectedValues.sliceMeter != nil {
		...
		meter, _ := p4rtClient.ReadMeterEntry(ctx, sliceTcMeter, expectedMeter.Index.GetIndex())
